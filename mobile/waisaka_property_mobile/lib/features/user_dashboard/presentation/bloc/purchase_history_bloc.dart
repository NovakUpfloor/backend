...
class PurchaseHistoryBloc extends Bloc<PurchaseHistoryEvent, PurchaseHistoryState> {
  ...
  PurchaseHistoryBloc({required UserDashboardRepository repository})
      : _repository = repository,
        super(PurchaseHistoryInitial()) {
    on<FetchPurchaseHistory>(_onFetchPurchaseHistory);
  }

  Future<void> _onFetchPurchaseHistory(
    FetchPurchaseHistory event,
    Emitter<PurchaseHistoryState> emit,
  ) async {
    emit(PurchaseHistoryLoading());
    try {
      final history = await _repository.fetchPurchaseHistory();
      emit(PurchaseHistoryLoadSuccess(history: history));
    } catch (e) {
      emit(PurchaseHistoryLoadFailure(error: e.toString()));
    }
  }
}
